// app/tuntunan-sholat/iqomah/page.jsx
'use client';
import { useEffect, useState, useRef } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { FaPlay, FaPause, FaVolumeUp, FaInfoCircle, FaBookOpen, FaShare, FaCopy } from 'react-icons/fa';
...
export default function IqomahPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('/api/tuntunan-sholat/iqomah');
        if (!response.ok) {
          throw new Error('Gagal memuat bacaan iqomah');
        }
  ...
  const toggleFullAudio = () => {
    if (isPlaying && currentAudio && activeItem === 'full') {
      currentAudio.pause();
      setIsPlaying(false);
      setActiveItem(null);
    } else {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `Pelajari bacaan iqomah: ${data.metadata.description}`,
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-blue-600 font-medium">Memuat bacaan iqomah...</p>
        </div>
      </div>
    );
  }
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-medium">Iqomah Lengkap</h3>
                  <p className="text-blue-100 text-sm opacity-80">Dengarkan seluruh bacaan iqomah</p>
                </div>
              </div>
              <button
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <ul className="space-y-2 pl-6">
                    {data.additionalInfo.perbedaanAdzanIqomah.map((item, index) => (
                      <motion.li 
                        key={index}
                        initial={{ opacity: 0, x: -10 }}
                        animate={{ opacity: 1, x: 0 }}
                  ...
                  <ul className="space-y-2 pl-6">
                    {data.additionalInfo.tataCara.map((item, index) => (
                      <motion.li 
                        ...
                        className="text-gray-600 relative before:content-['•'] before:absolute before:-left-4 before:text-teal-400"
                      >
                        {item}
                      </motion.li>
                    ))}
                ...
                <div className="bg-purple-50 p-4 rounded-xl border border-purple-100">
                  ...
                  <p className="text-purple-600">{data.additionalInfo.hukum}</p>
                </div>
